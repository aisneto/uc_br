...
read.uc <- function(id, var = 'area') {
  ...
  if (var== 'area') {
    uc <- sf::st_read(file.path(tempdir(), 'GEOFT_UNIDADE_CONSERVACAO.shp')) #le o shapefile das unidades de conservação brasil
    return(uc$geometry[uc$ID_UC0 == id]) #retorna o shapefile do id correspondente
  }
  if (var=='veg'){
    if (file.exists(file.path(tempdir(), 'vege_area', 'vege_area.shp')) == F ) { #verifica & baixa os dados
      download.data('https://geoftp.ibge.gov.br/informacoes_ambientais/vegetacao/vetores/escala_250_mil/versao_2021/vege_area.zip')
    }
    uc <- sf::st_read(file.path(tempdir(), 'GEOFT_UNIDADE_CONSERVACAO.shp')) #lê o shapefile das uc's brasil
    uc <- uc$geometry[uc$ID_UC0 == id ] #carrega o shapefile do id correspondente
    vg <- sf::st_read(file.path(tempdir(), 'vege_area', 'vege_area.shp')) #lê o shapefile de vegetação brasil
  ...
  if (var=='ped'){
    if (file.exists(file.path(tempdir(), 'pedo_area.shp')) == F ) { #verifica & baixa os dados
      download.data('https://geoftp.ibge.gov.br/informacoes_ambientais/pedologia/vetores/escala_250_mil/versao_2021/pedo_area.zip')
    }
    uc <- sf::st_read(file.path(tempdir(),'GEOFT_UNIDADE_CONSERVACAO.shp')) #lê o shapefile das uc's brasil
    uc <- uc$geometry[uc$ID_UC0 == id ] #carrega o shapefile do id correspondente
    pd <- sf::st_read(file.path(tempdir(), 'pedo_area.shp')) #lê o shapefile de pedologia brasil
    sf::sf_use_s2(FALSE) #parâmetro para não dar erro na interseção
    int <- sf::st_intersection(pd, uc) #intersecciona os shapefiles
    return(int[6]) #retorna a coluna de legenda
  }
  if (var=='geom'){
    if (file.exists(file.path(tempdir(), 'geom_area.shp', )) == F ) { #verifica & baixa os dados
    ...
    uc <- sf::st_read(file.path(tempdir(),'GEOFT_UNIDADE_CONSERVACAO.shp')) #lê o shapefile das uc's brasil
    uc <- uc$geometry[uc$ID_UC0 == id ] #carrega o shapefile do id correspondente
    geo <- sf::st_read(file.path(tempdir(), 'geom_area.shp')) #lê o shapefile de geomorfologia brasil
    sf::sf_use_s2(FALSE) #parâmetro para não dar erro na interseção
...
#corrigir
teste <- sf::st_read(file.path(tempdir(), 'vege_area', 'vege_area.shp'))
?file.path()
library(ggplot2)
a <- read.uc('320', 'veg')
ggplot(a) +
